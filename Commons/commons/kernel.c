...
void conexion_cpu(uint32_t id) {
	...
	log_info(logg, "Nueva CPU (%d) => [%s]", id, ids);
	free(ids);
}
void desconexion_cpu(uint32_t id) {
	...
	char* ids = ids_desde_lista(kernel_cpus_conectadas);
	log_info(logg, "Desconexión CPU (%d) => [%s]", id, ids);
	free(ids);
}
...
void conexion_consola(uint32_t id) {
	...
	log_info(logg, "Nueva Consola (%d) => [%s]", id, ids);
	free(ids);
}
...
void desconexion_consola(uint32_t id) {
	...
	char* ids = ids_desde_lista(kernel_consolas_conectadas);
	log_info(logg, "Desconexión Consola (%d) => [%s]", id, ids);
	free(ids);
}
...
void hilos(t_list* hilos) {
	...
	log_info(logg, resumen);
	free(resumen);
}

void instruccion_protegida(char* mnemonico, t_hilo* hilo) {
	log_info(logg, "El hilo { PID: %d, TID: %d } ejecutó la instrucción %s", hilo->pid, hilo->tid, mnemonico);
}
